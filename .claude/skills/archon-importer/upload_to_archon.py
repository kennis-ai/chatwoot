    ...
    async def upload_documents(self, file_paths: list[str]) -> dict[str, Any]:
        ...
        for i, file_path in enumerate(file_paths, 1):
            ...
            try:
                ...
                if result["success"] or result.get("success") != False:
                    successful += 1
                    # Skip progress polling - Archon processes uploads asynchronously
                    # Progress polling can cause hangs if the endpoint doesn't exist
                else:
                    failed += 1
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    if "project_id" in self.config:
                        data.add_field("project_id", self.config["project_id"])
