...
const values = computed(() =>
  props.items.map(item => parseFloat(item.item_details?.value) || 0)
);
...
const formattedAvg = computed(() =>
  values.value.length
    ? new Intl.NumberFormat('pt-BR', {
        style: 'currency',
        currency: 'BRL',
      }).format(values.value.reduce((a, b) => a + b, 0) / values.value.length)
    : '—'
);
...
const formattedMin = computed(() => {
  ...
  return new Intl.NumberFormat('pt-BR', {
    style: 'currency',
    currency: 'BRL',
  }).format(minValueFromItemsWithValue);
});
...
const oldestDate = computed(() =>
  sortedByDate.value[0]?.created_at
    ? new Date(sortedByDate.value[0].created_at)
    : null
);
const newestDate = computed(() =>
  sortedByDate.value[sortedByDate.value.length - 1]?.created_at
    ? new Date(sortedByDate.value[sortedByDate.value.length - 1].created_at)
    : null
);

// Cards sem agente
const itemsWithoutAgent = computed(() =>
  props.items.filter(item => !item.agent)
);
const totalWithoutAgent = computed(() =>
  itemsWithoutAgent.value.reduce(
...
const valueByStatus = computed(() => {
  ...
  props.items.forEach(item => {
    ...
    map[status] =
      (map[status] || 0) + (parseFloat(item.item_details?.value) || 0);
  });
  return map;
});
...
const percentByPriority = computed(() => {
  ...
  Object.keys(map).forEach(k => {
    map[k] = ((map[k] / total) * 100).toFixed(1);
  });
  return map;
});
...
const topAgents = computed(() =>
  Array.from(agentMap.value.values())
...
const busiestDay = computed(() =>
  sortedDays.value.length ? sortedDays.value[0].day : null
);
...
<template>
  <div
    class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-[4px]"
  >
    <div
      class="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-[110rem] p-8 overflow-y-auto max-h-[90vh] scrollbar-hidden"
    >
      <!-- Botão de fechar -->
      <button
        class="absolute top-4 right-4 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 text-2xl font-bold focus:outline-none"
        @click="onClose"
      >
        ×
      </button>
      ...
      <!-- Métricas em texto -->
      <div
        class="mb-6 text-slate-700 dark:text-slate-200 text-[15px] flex flex-col sm:flex-row flex-wrap gap-y-2 gap-x-4 items-start sm:items-center leading-tight"
      >
        <span
        ...
        <span class="inline-flex items-center gap-1"
          ><span class="w-2 h-2 rounded-full bg-slate-400 inline-block" /><span
            ...
            >Aberto:</span
          >
          <span class="font-semibold">{{ statusList[0].count }}</span></span
        >
        <span class="mx-2 hidden sm:inline">•</span>
        <span class="inline-flex items-center gap-1"
          ><span class="w-2 h-2 rounded-full bg-green-600 inline-block" /><span
            ...
            >Ganho:</span
          >
          <span class="font-semibold">{{ statusList[1].count }}</span></span
        >
        <span class="mx-2 hidden sm:inline">•</span>
        <span class="inline-flex items-center gap-1"
          ><span class="w-2 h-2 rounded-full bg-red-600 inline-block" /><span
            ...
            >Perdido:</span
          >
          <span class="font-semibold">{{ statusList[2].count }}</span></span
        >
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
        <div class="card-block card-header-agentes">
          <div class="card-block-header card-block-header-agentes">
            Principais agentes
          </div>
          <div class="flex flex-wrap gap-2">
            <div
              ...
              :key="agent.id"
              class="flex items-center gap-1 border border-slate-200 dark:border-slate-700 rounded-full px-2 py-1 bg-transparent text-xs text-slate-600 dark:text-slate-300"
            >
            ...
            <div
              v-if="extraAgents > 0"
              class="flex items-center gap-1 border border-slate-200 dark:border-slate-700 rounded-full px-2 py-1 text-xs font-semibold text-slate-500 dark:text-slate-400 bg-transparent"
            >
              +{{ extraAgents }} outros
            </div>
          </div>
        ...
        <div class="card-block card-header-prioridades">
          <div class="card-block-header card-block-header-prioridades">
            Prioridades
          </div>
          <div class="flex gap-2 flex-wrap">
            ...
            >
              <span
                :class="prio.class + ' w-2 h-2 rounded-full inline-block'"
              />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-1 mb-2">
        <div class="card-block card-header-status">
          ...
          <div class="flex gap-2 flex-wrap">
            ...
            >
              <span
                :class="stat.class + ' w-2 h-2 rounded-full inline-block'"
              />
        ...
        <div class="card-block card-header-sem-agente">
          <div class="card-block-header card-block-header-sem-agente">
            Cards sem agente
          </div>
          <div class="flex gap-2 flex-wrap">
            <div
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-1 mb-4">
        ...
        <div class="card-block card-header-valor-prioridade">
          <div class="card-block-header card-block-header-valor-prioridade">
            Valor por prioridade
          </div>
          <div class="flex gap-1 flex-wrap">
            <div
              v-for="(val, prio) in valueByPriority"
              :key="prio"
              :class="
        ...
        <div class="card-block card-header-valor-status">
          <div class="card-block-header card-block-header-valor-status">
            Valor por status
          </div>
          <div class="flex gap-2 flex-wrap">
            <div
              v-for="(val, status) in valueByStatus"
              :key="status"
              :class="
                'flex items-center gap-1 rounded-full px-2 py-1 bg-transparent text-xs font-semibold ' +
                (status === 'open'
                  ? 'border-slate-300 text-slate-700 bg-slate-100 dark:bg-slate-800/40 dark:text-slate-200'
                  : status === 'won'
                    ? 'border-green-300 text-green-700 bg-green-100 dark:bg-green-900/40 dark:text-green-300'
        ...
        <div class="card-block card-header-valor-agente">
          <div class="card-block-header card-block-header-valor-agente">
            Valor médio por agente
          </div>
          <div class="flex gap-2 flex-wrap">
            <div
              v-for="agent in Object.values(avgByAgent)"
              :key="agent.id"
              class="flex items-center gap-1 border border-slate-200 dark:border-slate-700 rounded-full px-2 py-1 bg-transparent text-xs text-slate-600 dark:text-slate-300"
            >
        ...
        <div class="card-block card-header-maior-menor-valor">
          <div class="card-block-header card-block-header-maior-menor-valor">
            Card de maior e menor valor
          </div>
          <div class="flex flex-col md:flex-row gap-4">
            <div
              v-if="maxItem"
              class="flex-1 flex items-center gap-2 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 bg-transparent text-xs text-slate-700 dark:text-slate-200"
            >
              <span class="font-semibold">Maior:</span>
              <span class="font-semibold">{{
                maxItem.item_details?.title
              }}</span>
              <span>|</span>
              <span>{{
                new Intl.NumberFormat('pt-BR', {
                  style: 'currency',
                  currency: 'BRL',
                }).format(parseFloat(maxItem.item_details?.value) || 0)
              }}</span>
              <span v-if="maxItem.agent">|</span>
              <span v-if="maxItem.agent">
                <Avatar
                  :name="maxItem.agent.name"
                  :src="maxItem.agent.avatar_url"
                  :size="18"
                />
                {{ maxItem.agent.name }}
              </span>
            ...
            >
              ...
              <span>{{
                new Intl.NumberFormat('pt-BR', {
                  style: 'currency',
                  currency: 'BRL',
                }).format(getMinItemValue)
              }}</span>
              <span v-if="minItem.agent">|</span>
              <span v-if="minItem.agent">
                <Avatar
                  :name="minItem.agent.name"
                  :src="minItem.agent.avatar_url"
                  :size="18"
                />
                {{ minItem.agent.name }}
              </span>
        ...
        <!-- Agendamentos -->
        <div
          class="card-block card-header-agendamentos col-span-1 md:col-span-2"
        >
          <div
            class="card-block-header card-block-header-agendamentos flex items-center gap-2"
          >
            <svg
          ...
          <div
            v-if="Object.keys(scheduleByDay).length"
          ...
          >
            <div class="flex flex-wrap gap-3 items-center">
              ...
              <span
                v-if="busiestDay"
                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 border border-green-300 text-green-700 dark:bg-green-900/40 dark:border-green-700 dark:text-green-300"
              >
                {{ busiestDay }}
                <span class="ml-1">({{ scheduleByDay[busiestDay] }})</span>
                <span
                  class="ml-2 text-slate-500 dark:text-slate-400 font-normal"
                  >{{ timeDiffLabel(busiestDay) }}</span
                >
              </span>
              <span
                class="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400"
              ...
              <span
                v-if="freestDay"
                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 border border-blue-300 text-blue-700 dark:bg-blue-900/40 dark:border-blue-700 dark:text-blue-300"
              >
                {{ freestDay }}
                <span class="ml-1">({{ scheduleByDay[freestDay] }})</span>
                <span
                  class="ml-2 text-slate-500 dark:text-slate-400 font-normal"
                  >{{ timeDiffLabel(freestDay) }}</span
                >
              </span>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
              <div>
                <div
                  class="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 mb-1"
                >
                  ...
                  >
                    <path
                      d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.967a1 1 0 00.95.69h4.18c.969 0 1.371 1.24.588 1.81l-3.388 2.46a1 1 0 00-.364 1.118l1.287 3.966c.3.922-.755 1.688-1.54 1.118l-3.388-2.46a1 1 0 00-1.175 0l-3.388 2.46c-.784.57-1.838-.196-1.54-1.118l1.287-3.966a1 1 0 00-.364-1.118l-3.388-2.46c-.783-.57-.38-1.81.588-1.81h4.18a1 1 0 00.95-.69l1.286-3.967z"
                    />
                  </svg>
                ...
                <div class="flex gap-1 flex-wrap">
                  <span
                    v-for="d in top3Busiest"
                    :key="d.day"
                    class="badge-glass bg-green-50 border-green-200 text-green-700 dark:bg-green-900/40 dark:border-green-700 dark:text-green-300"
                  >
                    {{ d.day }}
                    <span class="ml-1 font-semibold">({{ d.count }})</span>
                  </span>
                </div>
              </div>
              <div>
                ...
                <div class="flex gap-1 flex-wrap">
                  <span
                    v-for="d in top3Freest"
                    :key="d.day"
                    class="badge-glass bg-orange-50 border-orange-200 text-orange-700 dark:bg-orange-900/40 dark:border-orange-700 dark:text-orange-300"
                  >
                    {{ d.day }}
                    <span class="ml-1 font-semibold">({{ d.count }})</span>
                  </span>
...
.card-block-header > span,
.card-block-header > div,
.card-block-header > h3,
.card-block-header > h4,
