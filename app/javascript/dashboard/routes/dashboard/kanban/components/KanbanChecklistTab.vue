...
const getChecklistItemPriority = checklistItem => {
  ...
  const priorityMap = {
    ...
    low: {
      label: 'Baixa',
      color: 'green',
      iconSvg: '<path d="M7 13l3 3 3-3"/><path d="M7 6l3 3 3-3"/>',
    },
    medium: {
      label: 'Média',
      color: 'yellow',
      iconSvg: '<path d="M9 12l2 2 2-2"/><path d="M7 7l4 4 4-4"/>',
    },
    high: {
      label: 'Alta',
      color: 'orange',
      iconSvg: '<path d="M7 11l3-3 3 3"/><path d="M7 18l3-3 3 3"/>',
    },
    urgent: {
      label: 'Urgente',
      color: 'red',
...
const assignAgentToChecklistItem = async (checklistItem, agentId) => {
  try {
    await KanbanAPI.assignAgentToChecklistItem(
      props.item?.id,
      checklistItem.id,
      agentId
    );
...
const removeAgentFromChecklistItem = async checklistItem => {
  try {
    await KanbanAPI.removeAgentFromChecklistItem(
      props.item?.id,
      checklistItem.id
    );
  ...
  <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <div class="checklist-stats">
          <div class="flex items-center gap-4">
            <h3
              class="text-lg font-semibold text-slate-900 dark:text-slate-100 whitespace-nowrap flex items-center gap-2"
            >
              <svg
                ...
                class="lucide lucide-loader-icon lucide-loader text-slate-600 dark:text-slate-400"
              >
                <path d="M12 2v4" />
            ...
            <div class="flex items-center gap-2 flex-1">
              ...
              <span
                class="text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap"
              >
        ...
        <div class="flex-1">
          <label class="block text-sm text-slate-600 dark:text-slate-400 mb-1">
            <svg
              ...
              stroke-linecap="round"
              stroke-linejoin="round"
              class="inline mr-1 text-slate-600 dark:text-slate-400"
            >
              <path
    ...
    >
      ...
      >
        ...
        <div
          v-if="
            selectedChecklistItemForAgent &&
            getChecklistItemAgent(selectedChecklistItemForAgent)
          "
          class="mb-4"
        >
          <h4
          ...
          <div class="space-y-2">
            ...
            >
              ...
              <button
                ...
                @click="
                  removeAgentFromChecklistItem(selectedChecklistItemForAgent)
                "
              >
            ...
            <button
              ...
              :class="[
                selectedAgentId === agent.id
                  ...
                  : '',
                getChecklistItemAgent(selectedChecklistItemForAgent)?.id ===
                agent.id
                  ...
                  <button
                    ...
                    :class="{
                      'bg-green-500 border-green-500 text-white':
                        selectedChecklistItemDetails.completed,
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span class="text-slate-700 dark:text-slate-300">{{
                formatDate(selectedChecklistItemDetails.created_at)
              }}</span>
            </div>

            <div
              v-if="selectedChecklistItemDetails.updated_at"
            ...
            >
              ...
              <span class="text-slate-700 dark:text-slate-300">{{
                formatDate(selectedChecklistItemDetails.updated_at)
              }}</span>
            </div>

            <div
              v-if="selectedChecklistItemDetails.due_date"
              class="flex items-center gap-2"
            >
              <fluent-icon
                icon="calendar-clock"
              ...
              <span class="text-slate-700 dark:text-slate-300">{{
                formatDate(selectedChecklistItemDetails.due_date)
              }}</span>
            </div>
          </div>
              ...
              <span
                ...
                :class="{
                  'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300':
                    getChecklistItemPriority(selectedChecklistItemDetails)
                      .color === 'red',
          ...
          <div v-if="getChecklistItemAuthor(selectedChecklistItemDetails)">
            ...
            >
              <Avatar
                ...
                "
                :src="
                  getChecklistItemAuthor(selectedChecklistItemDetails)
                    .avatar_url
                "
                :size="32"
              />
          ...
          <div v-if="getChecklistItemAgent(selectedChecklistItemDetails)">
            ...
            >
              <Avatar
                ...
                :src="
                  getChecklistItemAgent(selectedChecklistItemDetails).avatar_url
                "
                :size="32"
              />
              <div>
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-2">
                <div
                  ...
                  v-html="renderMarkdown(checklistItem.text)"
                />
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <div
                      v-if="
                        ...
                        'Nenhuma'
                      "
                      class="checklist-priority"
                    >
                      <span
                        class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full"
                        :class="{
                          'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300':
                            getChecklistItemPriority(checklistItem).color ===
                            'red',
                          'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300':
                            getChecklistItemPriority(checklistItem).color ===
                            'orange',
                      ...
                      >
                        <svg
                          ...
                          class="flex-shrink-0"
                          v-html="
                            getChecklistItemPriority(checklistItem).iconSvg
                          "
                        />
                        {{ getChecklistItemPriority(checklistItem).label }}
                      </span>
                    ...
                    <span class="checklist-author text-xs">
                      {{
                        getChecklistItemAuthor(checklistItem)?.name ||
                        'Sem autor'
                      }}
                    </span>
                    ...
                    <button
                      ...
                      @click.stop="
                        openAgentAssignModalForChecklistItem(checklistItem)
                      "
                    >
              ...
              <!-- Prazo do item -->
              <div
                v-if="checklistItem.due_date"
                class="checklist-due-date-container mt-2"
              >
                <span
                  ...
                  :class="{
                    'text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800':
                      new Date(checklistItem.due_date) < new Date() &&
                      !checklistItem.completed,
                    'text-yellow-700 dark:text-yellow-300 bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800':
                      new Date(checklistItem.due_date) >= new Date() &&
                      new Date(checklistItem.due_date) <=
                        new Date(Date.now() + 24 * 60 * 60 * 1000) &&
                      !checklistItem.completed,
                    'text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700':
                      new Date(checklistItem.due_date) >
                        new Date(Date.now() + 24 * 60 * 60 * 1000) ||
                      checklistItem.completed,
                  }"
                >
                  ...
                  {{ t('KANBAN.FORM.CHECKLIST.DUE_DATE') }}
                  {{ formatDate(checklistItem.due_date) }}
                </span>
              </div>
            </div>
          </div>
