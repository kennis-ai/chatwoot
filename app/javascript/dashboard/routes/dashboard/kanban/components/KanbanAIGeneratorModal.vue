...
const handleConfirmGeneration = async () => {
  try {
    ...
    useAlert(
      t('KANBAN.AI.ITEMS_GENERATED', { count: generatedItems.value.length })
    );
    ...
    <div class="w-full max-w-4xl">
      ...
      <div class="border-b border-slate-200 p-6 dark:border-slate-700">
        <div class="flex items-center gap-3">
          ...
          >
            <i
              class="i-lucide-sparkles h-6 w-6 text-purple-600 dark:text-purple-300"
            />
          </div>
          <div>
        ...
        <div class="space-y-6">
          ...
          >
            <div class="flex items-start gap-3">
              <i
                class="i-lucide-alert-triangle h-5 w-5 text-yellow-600 dark:text-yellow-400"
              />
              <div>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <button
                class="flex flex-col items-center gap-3 rounded-lg border-2 p-6 transition-all"
                :class="[
              ...
              >
                <i
                  class="i-lucide-users h-8 w-8 text-blue-600 dark:text-blue-400"
                />
              ...
              >
                <i
                  class="i-lucide-message-circle h-8 w-8 text-blue-600 dark:text-blue-400"
                />
                <div class="text-center">
                  <p class="font-semibold text-slate-900 dark:text-slate-25">
          ...
          <div>
            ...
            >
              <option value="last_7_days">
                {{ t('KANBAN.AI.LAST_7_DAYS') }}
              </option>
              <option value="last_30_days">
                {{ t('KANBAN.AI.LAST_30_DAYS') }}
              </option>
              <option value="last_90_days">
                {{ t('KANBAN.AI.LAST_90_DAYS') }}
              </option>
          ...
          >
            <div class="mb-2 flex items-start justify-between">
              ...
              <span
                ...
                :class="getPriorityColor(item.item_details.priority)"
              >
                <i
                  class="h-3 w-3"
                  :class="getPriorityIcon(item.item_details.priority)"
                />
                {{
                  t(
                    `KANBAN.PRIORITY.${item.item_details.priority.toUpperCase()}`
                  )
                }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-2 text-xs text-slate-500">
              <span
                class="rounded-full bg-slate-100 px-2 py-1 dark:bg-slate-700"
              >
                {{ item.funnel_stage }}
              </span>
      ...
      <!-- Step 3: Success -->
      <div v-else-if="step === 3" class="p-12 text-center">
        <div
          class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-green-100 dark:bg-green-900"
        >
          <i
            class="i-lucide-check h-8 w-8 text-green-600 dark:text-green-300"
          />
        </div>
        <h3
          class="mb-2 text-xl font-semibold text-slate-900 dark:text-slate-25"
        >
